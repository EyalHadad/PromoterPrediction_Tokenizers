...
This script processes cluster files generated by CD-HIT-EST to create data splits without leakage.
Before running this script, you must generate the .clstr files by running the following command 
in your terminal for each organism's FASTA file:

Command:
    cd-hit-est -i input.fasta -o output_name -c 0.8 -n 5 -M 16000 -T 0

Parameters:
    -i : Input FASTA file (positive promoters)
    -o : Output filename (files should be saved in 'new_raw_data/cd_hit80/')
    -c 0.8 : Sequence identity threshold (80%)
    -n 5 : Word size (required for 0.8 threshold)
